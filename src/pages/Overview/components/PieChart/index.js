import React, { useState } from "react";
import { useTheme } from "../../../../contexts/ThemeContext";
import "./PieChart.css";

const PieChart = () => {
  const { isDarkMode } = useTheme();
  const [hoveredSegment, setHoveredSegment] = useState(null);

  // Sample data for the pie chart - using exact colors from svgForpiechartPortions.txt
  const pieData = [
    {
      id: 1,
      label: "Direct",
      value: 300.56,
      percentage: 38.6,
      color: "#B1E3FF", // Blue
      dotColor: "#B1E3FF",
      path: "M17.0121 18.3464C16.367 18.7927 15.7635 19.3216 15.2168 19.9324C10.6473 25.0377 6.93145 30.94 4.30211 37.4135C2.18604 42.6234 4.67487 48.57 9.86107 50.6957C15.0473 52.8215 20.9669 50.3213 23.083 45.1114C24.816 40.8447 27.2708 36.941 30.3004 33.5562C30.3521 33.4984 30.4031 33.4401 30.4533 33.3814C26.2306 33.1582 22.1506 31.0505 19.5441 27.2966C17.6541 24.5746 16.8402 21.4214 17.0121 18.3464Z",
    },
    {
      id: 2,
      label: "Affiliate",
      value: 135.18,
      percentage: 17.4,
      color: "#95A4FC", // Purple
      dotColor: "#95A4FC",
      path: "M36.9918 27.4926C43.3945 23.0062 51.1699 20.3765 59.5871 20.3765C68.0043 20.3765 75.7798 23.0062 82.1825 27.4926C82.3342 27.5989 82.4877 27.7005 82.643 27.7973C82.3329 23.5486 83.9128 19.2063 87.3104 16.1376C89.73 13.9521 92.6913 12.756 95.7028 12.5229C95.1532 11.8765 94.5134 11.2892 93.7869 10.7801C84.0906 3.98588 72.287 0 59.5871 0C46.8872 0 35.0837 3.98588 25.3873 10.7801C20.7932 13.9992 19.6667 20.35 22.8712 24.9651C26.0757 29.5801 32.3977 30.7117 36.9918 27.4926Z",
    },
    {
      id: 3,
      label: "Sponsored",
      value: 154.02,
      percentage: 19.8,
      color: "#BAEDBD", // Green
      dotColor: "#BAEDBD",
      path: "M1.81254 47.2351C3.26614 50.4036 5.87327 53.0469 9.34258 54.4689C13.1587 56.0331 17.2585 55.8082 20.7197 54.1838C20.4333 56.1121 20.2841 58.0908 20.2841 60.1106C20.2841 72.964 26.3595 84.392 35.7831 91.6547C36.2843 91.894 36.7726 92.1779 37.2428 92.5073C43.6455 96.9937 51.4209 99.6235 59.8382 99.6235C63.4875 99.6235 67.0059 99.1297 70.3383 98.2109C75.7398 96.7216 81.3205 99.913 82.803 105.339C84.2856 110.765 81.1086 116.371 75.7071 117.861C70.6418 119.257 65.3183 120 59.8382 120C56.1352 120 52.5085 119.661 48.988 119.013C49.0629 119.102 49.1388 119.19 49.2158 119.277C21.2354 114.241 0 89.6672 0 60.1106C0 56.7798 0.270663 53.505 0.793098 50.3091C0.973374 49.2062 1.32363 48.1741 1.81254 47.2351Z",
    },
    {
      id: 4,
      label: "E-mail",
      value: 48.96,
      percentage: 6.3,
      color: isDarkMode ? "#C6C7F8" : "#1C1C1C", // Black in light theme, purple in dark theme
      dotColor: isDarkMode ? "#C6C7F8" : "#1C1C1C",
      path: "M104.67 19.9323C100.925 15.7481 94.5122 15.406 90.347 19.1681C86.1818 22.9302 85.8411 29.372 89.5862 33.5562C95.8915 40.6008 99.7159 49.895 99.7159 60.1106C99.7159 75.22 91.3206 88.3596 78.9604 95.079C82.6329 96.7797 85.561 100.036 86.7151 104.261C87.6887 107.824 87.2269 111.443 85.6817 114.496C105.965 104.874 120 84.139 120 60.1106C120 44.6722 114.193 30.5722 104.67 19.9323Z",
    },
  ];

  return (
    <div className={`pie-chart ${isDarkMode ? "dark" : "light"}`}>
      {/* Header */}
      <div className="pie-chart-header">
        <h3 className="pie-chart-title">Total Sales</h3>
      </div>

      {/* Pie Chart Container */}
      <div className="pie-chart-container">
        <svg
          width="120"
          height="120"
          viewBox="0 0 120 120"
          className="pie-chart-svg"
        >
          {pieData.map((segment) => (
            <g key={segment.id}>
              <path
                d={segment.path}
                fill={segment.color}
                fillRule="evenodd"
                clipRule="evenodd"
                className={`pie-segment ${
                  hoveredSegment === segment.id ? "hovered" : ""
                }`}
                onMouseEnter={() => setHoveredSegment(segment.id)}
                onMouseLeave={() => setHoveredSegment(null)}
                style={{
                  cursor: "pointer",
                  transition: "all 0.3s ease",
                  transform:
                    hoveredSegment === segment.id ? "scale(1.02)" : "scale(1)",
                  transformOrigin: "60px 60px",
                  filter:
                    hoveredSegment === segment.id
                      ? "brightness(1.1)"
                      : "brightness(1)",
                }}
              />
            </g>
          ))}
        </svg>
      </div>

      {/* Legend */}
      <div className="pie-chart-legend">
        {pieData.map((item) => (
          <div key={item.id} className="legend-item">
            <div className="legend-tag-container">
              <div
                className="legend-dot"
                style={{ backgroundColor: item.dotColor }}
              />
              <span className="legend-label">{item.label}</span>
            </div>
            <span className="legend-amount">${item.value.toFixed(2)}</span>
          </div>
        ))}
      </div>

      {/* Tooltip */}
      {hoveredSegment && (
        <div className="pie-chart-tooltip">
          <div className="tooltip-content">
            <div className="tooltip-label">
              {pieData.find((item) => item.id === hoveredSegment)?.label}
            </div>
            <div className="tooltip-value">
              $
              {pieData
                .find((item) => item.id === hoveredSegment)
                ?.value.toFixed(2)}
            </div>
            <div className="tooltip-percentage">
              {pieData.find((item) => item.id === hoveredSegment)?.percentage}%
            </div>
          </div>
        </div>
      )}
    </div>
  );
};

export default PieChart;
